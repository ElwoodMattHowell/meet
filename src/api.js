...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch('https://c4sgbozhub.execute-api.us-east-1.amazonaws.com/dev/api/token' + '/' + encodeCode
  ).then((res) => {
    return res.json();
  }).catch((error) => error);
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        "https://c4sgbozhub.execute-api.us-east-1.amazonaws.com/dev/api/get-auth-url"
      );
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = 'https://c4sgbozhub.execute-api.us-east-1.amazonaws.com/dev/api/get-events' + '/' + token;
